...
const convertRolesToSections = (roles: Role[]): Section[] => {
  return roles.flatMap(role => {
    const roleProps = { role: role }; // Props to pass to RolePage component
    return [
      {
        id: role.id.toString(),
...
const MainPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:8000/core/roles/');
  ...
  const sections = useMemo(() => {
    return [
      ...
      ...convertRolesToSections(roles), // Spread the array returned by convertRolesToSections
